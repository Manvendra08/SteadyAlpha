...
class OptionsEngine:
    ...
    def analyze_oi_walls(self, spot_price: float, option_chain: pd.DataFrame) -> Dict[str, Any]:
        """
        ...
        dist_to_call = (call_wall - spot_price) / spot_price if call_wall else 1.0
        dist_to_put = (spot_price - put_wall) / spot_price if put_wall else 1.0
    ...
    def run(self, spot_price: float, iv_series: pd.Series, 
            option_chain: pd.DataFrame, pcr_oi_series: pd.Series) -> Dict[str, Any]:
        """
        ...
        options_state = {
            'iv_rank': iv_rank,
            'oi_pcr': round(float(pcr_oi_series.iloc[-1]), 4) if not pcr_oi_series.empty else 0.0,
